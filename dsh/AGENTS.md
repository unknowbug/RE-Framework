...
2. 跑自检确认基线全绿：`pwsh dsh/scripts/selfcheck.ps1`（六段：工具链 / 技能 manifest / 安装产物 / 插件 schema / preset 行解析门禁 / patch 层 row 原语）。
...
| `scripts/selfcheck.ps1` | 六段自检（工具链 / 技能 manifest / **安装产物对账** / 插件 schema / preset 行解析门禁 / patch 层 row 原语） | 维护工具 |
| `scripts/patch_layer.py` | **patch 层共享原语**（`--has-row` 只读判定；`--remove-row` 按 row id 文本级摘除，保 BOM/行尾/注释/其他 row，仅真删到 row 时写盘 + `.bak-ref-install` 备份）——install 与 selfcheck 共用，2026-09-18 新增 | 维护工具 |
...
| `tests/test_home_patch.py` | patch 层 row 原语回归（13 例：**存在≠违规** / 注释提及不算 row / 按 entry 粒度只删本框架 entry（同一条 insert 里的他框架 entry 存活）且其余逐字节不变 / 无 row 零写入且无备份 / 删空后补 `[]` 保证层仍可启动 / inline 形态可识别 / 备份为原始字节）——2026-09-18 事故门禁 | 维护测试 |
...
1. **自检全绿**：任何改动必须 `dsh/scripts/selfcheck.ps1` 全绿（六段：工具链 / 技能 manifest / 安装产物 / 插件 schema / preset 行解析门禁 / patch 层 row 原语）。
