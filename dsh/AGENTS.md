...
| `~/.dsh/plugins/re-framework/` + `~/.dsh/cordis.patch.yml` | **用户级全局工具**（项目侧 ref_*，home patch 热生效） | install.ps1 生成 |
...
7. **可见性纪律（v2.1 修订）**：技能装**用户级全局**（`~/.dsh/skills/ref-*`）+ preset 内嵌双路径——任何项目会话可加载，不依赖选择 re-framework preset 或工作目录；项目侧工具（status/init/merge_index/install）经 **home patch**（`~/.dsh/cordis.patch.yml`）全局，维护工具（manifest_validate）仅 preset——两处注册名不重叠；多框架共存靠前缀命名空间（ref-*/ref_* 与 anchor-*/anchorlaw_*）+ 插件子目录（`~/.dsh/plugins/<framework>/`）。
