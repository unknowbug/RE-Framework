...
| 2026-08-14 | 可见性设计修订（用户需求） | 技能：preset-only → **用户级全局**（~/.dsh/skills/ref-*）+ preset 内嵌双路径；工具分层：项目侧（status/init/merge_index/install）经 **home patch**（~/.dsh/cordis.patch.yml）用户级全局，维护工具（manifest_validate）仅 preset；插件支持 config.tools 子集注册（两处不重叠）；install.ps1/selfcheck.ps1/文档同步更新 | 0 |
