...
whenToUse: confirmed 授予前/重大转向/收尾交付 MUST、candidate 授予 SHOULD 时（DSH 中经 subagent 工具隔离执行，只出审查意见不改 status）
---
...
9. **判据前置集**（Anchorlaw v0.22 §15.1/§15.4）: 被引用的验收判据其 `preconditions`（key/expected/check）当前是否仍被核？前置失效 → 意见标注「该判据 suspended、引用它的结论前提已失效待复核」，并**只出意见**：不得改任何 status、不得撤销 `confirmed`（§1.1 + §16.1 confirm hook 是硬边界）
10. **验证副作用与逆**（Anchorlaw v0.22 §9.8，spec §5.6）: 本轮验证动作的 in-place 副作用（覆盖日志/原地重建 index/改门控默认值/污染执行体）是否登记了**可寻址的逆**，或给出显式不可逆声明及原因？未登记 → 意见中标注；**不得要求自动回退**（失败轮证据价值更高）
11. **等价档位与 `S`**（Anchorlaw v0.22 §9.7.1）: 跨载体/跨形态的量化结论是否声明了等价档位（E1/E2/E3）与共同观测 key 集 `S`，并说明 `S` 之外无分支？未声明或用了 §9.7.1 无效声明清单中的形态（计数恒等当集合恒等 / 同口径当无伪差 / 合理解释当验证 / 前提存在当满足）→ 意见中标注
12. **halt 终态性**（Anchorlaw v0.22 §15.4 PI-1）: 若本轮是 gate C 触发的 halt，是否以**机械信号**终止（非零退出或等价 host 标记）且**未被静默继承**为下一轮基底？被继承 → 判为流程不变量破裂并驳回
