...
| 3 个工具（status/merge_index/init） | 仅 re-framework preset（`ref_*` 工具是框架 python 脚本的包装；其他会话用 pwsh 直接跑脚本；manifest_validate/install 随 Reasonix 归档退役） |
| Phase 0-3 工作流 + 执行强制链（scout/fan-out/judge/knowledge） | agent preset `re-framework`（人格 + subagent 隔离） |
| 技能一致性守护 | `tests/test_manifest.py`（DSH 命名 + frontmatter + 技能集 + 交叉引用）+ `SYNC.md`（溯源）；`dsh/skills/` 是单一事实源（`sync_skills.py` 随 Reasonix 归档退役） |
...
- **工具仅 preset**：3 个 `ref_*` 工具（status / merge_index / init）只在 re-framework preset 会话出现——它们是框架 python 脚本（`scripts/merge_index.py` 等）的便利包装，其他会话直接用 pwsh 跑脚本即可，无需全局工具组。
- **权限边界**（实测）：跨工作区**读**放行、**写**仅限会话工作区——项目侧操作（读框架源 + 写目标项目工作区内）pwsh 直接可跑。

## 快速开始

```powershell
# Windows（无 pwsh 时用 powershell.exe -NoProfile -ExecutionPolicy Bypass -File <脚本>）
...
```bash
# Linux / macOS / 麒麟（无需 PowerShell）
bash dsh/scripts/install.sh
```

装完后，**任何 preset 会话**（standard 等）都能在技能目录看到 17 个 ref-* 技能按需加载；选 **re-framework** preset 另有完整工作台（人格 + 3 个 `ref_*` 工具）。工作目录指向项目本身（如 `E:\PYTHON\CoreSwap`）即可，无需指向本仓库。

## 目录结构

```
dsh/
├── skills/                # 17 个技能事实源（单一事实源，直接维护）
├── plugins/               # 工具插件事实源（re-framework-tools.js，config.tools 控制注册子集）
├── preset/                # agent preset 组合源（agent.cordis.yml + preset.yml）
├── scripts/               # install.ps1 + install.sh（部署）/ selfcheck.ps1（自检）/ gen_cheatsheet.py
...
- **单一事实源**：`dsh/skills/` 是技能唯一事实源（直接维护，无上游派生；Reasonix 归档不参与同步）
...
- 验证协议引用 [Anchorlaw v0.20](https://github.com/unknowbug/anchorlaw)（协议引用，不复制实现；swe 模块可选装 anchorlaw CLI）
