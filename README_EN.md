...
> Verification protocol: references [Anchorlaw Protocol v0.22](https://github.com/unknowbug/anchorlaw) (MIT, protocol reference — no implementation copied).
...
├── spec/                      # Framework protocol (iron rules/workflow/artifacts/knowledge/versioning) + Anchorlaw v0.22 reference
...
|---------|------|---------|
| v2.7 | 2026-09-18 | **Patch-layer judgement fixed (judge the row, never the file) + destructive write removed**: `selfcheck.ps1` failed whenever `$DSH_HOME/cordis.patch.yml` merely **existed**, and `install.ps1` **deleted the whole file** when it carried this framework's row. That file is a legal home-level patch layer (applied after the per-profile layer and therefore outranking it — see the `@deepseek-ai/dsh-app-boot` README and `homePatchPath()` in both 0.1.5-rc.2 and 0.1.6-alpha.2), and it carries other frameworks' rows plus machine-local settings. Fix: (1) new shared primitive `dsh/scripts/patch_layer.py` (`--has-row` / `--remove-row`; text-level, preserves BOM/line endings/comments, writes and backs up `.bak-ref-install` only when a row is really removed, atomic replace, diagnostics on stdout to dodge PowerShell 5.1 stderr promotion); (2) both `selfcheck.ps1` and `install.ps1` now use that one primitive for the home layer and every profile layer (previously content-based for profiles vs existence-based for home — the exact split that caused the bug); (3) new selfcheck section 6 `dsh/tests/test_home_patch.py` (13 cases locking "exists is not a violation", "entry-level removal keeps a sibling entry from another framework", "zero writes when absent", "append `[]` after an emptying removal so the layer still boots"). Self-check five sections → six | 0 |
| v2.6 | 2026-09-17 | **Anchorlaw v0.22 substantive clause upgrade + CoreSwap paper-study absorption**: (1) reference v0.21 → **v0.22** — v0.22 adds **five clauses** (§9 +37 lines, §14 +34, §15 +75), so this is a **substantive upgrade, not a bare version bump**; all five are landed (§9.8 side-effect boundary and inverse registration → spec §5.6 + core-artifact; §15.4 criterion preconditions → spec §4.5 + core-judge; §9.7.1 equivalence tiers → re-lift; §15.4 PI-1 halt-is-terminal → spec §4.5 + core-fanout convergence completeness; §14.7 reference integrity → spec §3 check action). (2) **Installed artifacts gain an identity** — `install.ps1` writes a sha256 manifest and `selfcheck.ps1` section 3 upgrades from counting directories to **content reconciliation** (MISSING/DRIFT/ORPHAN); two pre-existing holes closed: orphan cleanup in the user-global tree (scoped to this framework's namespace, **never touching `anchor-*`**) and a backup before rewriting the profile patch. (3) New trigger-coverage gate `audit_trigger_coverage.mjs` (**judges "is there an artifact after a trigger", not "should it have triggered"**; measured false-positive rate **1.1%**). (4) Handoff claim evidence grades (numeric inheritable / qualitative must re-sample the raw log / anchor must be self-checked with its owner tool), **scoped to semantic handoffs** to avoid over-engineering |
