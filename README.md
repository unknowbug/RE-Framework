...
> 验证协议继承 [Anchorlaw Protocol v0.15](https://github.com/unknowbug/anchorlaw)（MIT，协议引用，不复制实现）。
...
| 验证协议 | 引用 Practify（已死链） | 协议引用 Anchorlaw v0.15（单一事实源） |
...
- **协议引用**：`@anchor.test` / `@anchor.idk`、source 规则、staleness、噪声卡、降级验证（uncompilable_functions.yaml / retry cap）——全部指向 [Anchorlaw v0.15](https://github.com/unknowbug/anchorlaw)，本框架不复制实现。
...
- 验证协议：[Anchorlaw Protocol v0.15](https://github.com/unknowbug/anchorlaw)（MIT，由 Practify 更名而来）
