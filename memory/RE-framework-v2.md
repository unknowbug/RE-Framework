...
- `spec/engineering-framework-v1.md` — 核心协议：铁律（领域无关）+ 模块化接口契约（模块定义/探测自适应/卸载保证/manifest 校验）+ Anchorlaw v0.15 四接口面引用
...
**验证协议:** 引用 Anchorlaw v0.15（MIT，由 Practify 更名而来），不复制实现——见 `spec/engineering-framework-v1.md` §3。
